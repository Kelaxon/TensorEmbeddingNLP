...
matrix_info = load('tmp_matrix_info.mat');

sp_A = sparse(double(matrix_info.index_i+1), double(matrix_info.index_j+1), double(matrix_info.val));
...
%% Prior beliefs
pseudopriors = double(matrix_info.label');
...
final_labels(idx_bad,1) = 0;
final_labels(idx_good,1) = 1;

%% SAVE THE RESULTS
save('tmp_label_mat.mat', 'final_labels');
disp('* Final node labels saved in "finalLabels.out". *');
disp('* Format of file: one label (0 or 1) per line,  *');
